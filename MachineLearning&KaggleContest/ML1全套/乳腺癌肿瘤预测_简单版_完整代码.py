...
Clump_Thickness = plt.scatter( df_test_positive['Clump Thickness'],
                               df_test_positive['Cell Size'],
                               marker = 'o',
                               s = 200,
                               c = 'red' )
# 绘制图1-1中恶性肿瘤样本点，标记为黑色的x，并且保存绘图句柄Cell_Size
Cell_Size = plt.scatter( df_test_negative['Clump Thickness'],
...
lr.fit( df_train[['Clump Thickness', 'Cell Size']][:10],
        df_train['Type'][:10] )
print( 'Testing accuracy (10 train_set samples):',
...
Train_10_ly = (-Train_10_intercept - Train_10_lx * Train_10_coef[0])\
              / Train_10_coef[1]
# 绘制图1-3
plt.figure('图1-3')
...
print( 'Testing accuracy (all train_set samples):',
       lr.score( df_test[['Clump Thickness', 'Cell Size']],
                 df_test['Type'] ) )
...
Train_all_ly = (-Train_all_intercept - Train_all_lx * Train_all_coef[0])\
               / Train_all_coef[1]
# 绘制图1-4
plt.figure('图1-4')
