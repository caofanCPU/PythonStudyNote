...
# 输出决策树模型依靠所有特征后的性能评分
print('The accuracy of DecisionTreeClassifier[100%] is:', dtc.score(X_test_vec, y_test))
...
print('The accuracy of DecisionTreeClassifier[20%] is:', dtc_fs.score(X_test_fs, y_test))
...
for i in percentiles:
    ...
    fs_percentiles = feature_selection.SelectPercentile(feature_selection.chi2, percentile=i)
    # 依次应用特征筛选比例对象fs_percentiles，适应转化训练集特征向量
    X_train_fs_percentiles = fs_percentiles.fit_transform(X_train_vec, y_train)
    # 决策树模型对象为dtc_fs_percentiles
    # 在决策树模型对象中，进行参数训练，并采用5折交叉验证
    # 此处训练参数为( X_train_fs_percentiles, y_train )
    scores = cross_val_score(dtc_fs_percentiles, X_train_fs_percentiles, y_train, cv=5)
...
fs_opt_percentiles = feature_selection.SelectPercentile(feature_selection.chi2, percentile=percentiles[opt[0]])
X_train_fs_opt_percentiles = fs_opt_percentiles.fit_transform(X_train_vec, y_train)
...
print('The accuracy of DecisionTreeClassifier[%d%%] is:' % percentiles[opt[0]], dtc_fs_opt_percentiles.score(X_test_fs_opt_percentiles, y_test))
