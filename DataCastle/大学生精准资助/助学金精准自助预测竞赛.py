...
train = pd.read_table( 'G:/P_Anaconda3-4.2.0/PA-WORK/'\
                       '助学金精准自助预测竞赛/train/subsidy_train.txt',
                       sep = ',',
                       header = -1 )
...
test = pd.read_table( 'G:/P_Anaconda3-4.2.0/PA-WORK/'\
                      '助学金精准自助预测竞赛/test/studentID_test.txt',
                      sep = ',',
                      header = -1 )
...
score_train = pd.read_table( 'G:/P_Anaconda3-4.2.0/PA-WORK/'\
                             '助学金精准自助预测竞赛/train/score_train.txt',
                             sep = ',',
                             header = -1 )
...
score_test = pd.read_table( 'G:/P_Anaconda3-4.2.0/PA-WORK/'\
                            '助学金精准自助预测竞赛/test/score_test.txt',
                            sep = ',',
                            header = -1 )
...
college = pd.DataFrame( score_train_test.groupby( ['college'] )\
                        ['score'].max(  ) )
...
score_train_test = pd.merge( score_train_test, 
                             college,
                             how = 'left',
                             on = 'college' )
...
score_train_test['order'] =\
score_train_test['score'] / score_train_test['num']
...
card_test = pd.read_table( 'G:/P_Anaconda3-4.2.0/PA-WORK/'\
                           '助学金精准自助预测竞赛/test/card_test.txt',
                           sep = ',',
                           header = -1 )
"""
...
card = pd.DataFrame( card_train_test.groupby( ['id'] )\
                     ['consume'].count(  ) )
...
card['consumesum'] = card_train_test.groupby( ['id'] )\
                                    ['amount'].sum(  )
card['consumeavg'] = card_train_test.groupby( ['id'] )\
                                    ['amount'].mean(  )
card['consumemax'] = card_train_test.groupby( ['id'] )\
                                    ['amount'].max(  )
card['remaindersum'] = card_train_test.groupby( ['id'] )\
                                      ['remainder'].sum(  )
card['remainderavg'] = card_train_test.groupby( ['id'] )\
                                      ['remainder'].mean(  )
card['remaindermax'] = card_train_test.groupby( ['id'] )\
                                      ['remainder'].max(  )
...
gbc = GradientBoostingClassifier( n_estimators = 200,
                                  random_state = 2016 )
gbc = gbc.fit( CE_train[predictors],
               CE_train[target] )
...
# 输出梯度提升模型结果
print( '1000--' + 
       str( len( test_gbc_result[test_gbc_result.subsidy == 1000] ) ) +
       ':741' )
print( '1500--' + 
       str( len(test_gbc_result[test_gbc_result.subsidy == 1500] ) ) + 
       ':465' )
print( '2000--' + 
       str( len( test_gbc_result[test_gbc_result.subsidy == 2000] ) ) + 
       ':354' )

test_gbc_result.to_csv( 'G:/P_Anaconda3-4.2.0/PA-WORK/助学金精准自助预测竞赛/'\
                        'save_output/test_clf_result2016.csv',
                    index = False )
####################################################
...
rfc = RandomForestClassifier( n_estimators = 500,
                              random_state = 2016)
rfc = rfc.fit( CE_train[predictors],
               CE_train[target] )
...
# 输出随机森林模型结果
print( '1000--' + 
       str( len( test_rfc_result[test_rfc_result.subsidy == 1000] ) ) +
       ':741' )
print( '1500--' + 
       str( len(test_rfc_result[test_rfc_result.subsidy == 1500] ) ) + 
       ':465' )
print( '2000--' + 
       str( len( test_rfc_result[test_rfc_result.subsidy == 2000] ) ) + 
       ':354' )

test_rfc_result.to_csv( 'G:/P_Anaconda3-4.2.0/PA-WORK/助学金精准自助预测竞赛/'\
                        'save_output/test_rfc_result2016.csv',
                    index = False )
#####################################################
# 运行结果
